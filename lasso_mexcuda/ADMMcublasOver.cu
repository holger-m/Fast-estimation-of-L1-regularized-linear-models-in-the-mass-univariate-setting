...
// compile: system('nvcc -c ADMMcublasOver.cu');
        
// alternatively (on Windows): system('nvcc -c ADMMcublasOver.cu -ccbin "C:\Program Files (x86)\Microsoft Visual Studio\2017\Community\VC\Tools\MSVC\14.16.27023\bin\Hostx64\x64"');
...
void ADMM_cublas_over(int N_i, int N_j, int N_n, int N_batch, int n_iter_max, float *z_in_host, float *u_in_host, float *lambda_value_in_host, float *Atb_active_in_host, float *At_LU_inv_in_host, float *A_in_host, float *tol_value_in_host, float *z_host_out, float *u_host_out, bool *error_flag_max_iter)
{
    ...
    float *dAt_LU_inv_in;
    cudaMalloc(&dAt_LU_inv_in, (N_i*N_j)*sizeof(float));

    float *dA_in;
    cudaMalloc(&dA_in, (N_i*N_j)*sizeof(float));
    ...
    cudaMemcpy(dAt_LU_inv_in, At_LU_inv_in_host, (N_i*N_j)*sizeof(float), cudaMemcpyHostToDevice);
    cudaMemcpy(dA_in, A_in_host, (N_i*N_j)*sizeof(float), cudaMemcpyHostToDevice);
    ...
    float *dAq_out;
    cudaMalloc(&dAq_out, (N_i*N_batch)*sizeof(float));
    ...
    for (iter_no = 0; iter_no < n_iter_max; iter_no++) {
        ...
        cublasSgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N, N_i, N_batch, N_j, &scalar_p1, dA_in, N_i, dq_out, N_j, &scalar_0, dAq_out, N_i);

        cublasSgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N, N_j, N_batch, N_i, &scalar_m1, dAt_LU_inv_in, N_j, dAq_out, N_i, &scalar_0, dx_out, N_j);

        cublasSaxpy(handle, N_n, &scalar_p1, dq_out, 1, dx_out, 1);
